...
import { AlignmentLine, AntetypeWeb } from './viewer.js';
import { GDModelObject, GDState, GDVectorCellType, GDMouseClickEventType, CPColor } from './model.js';
import { sizeHighlightCell, globalBoundsOfElement, gdPostViewerMessage } from './utils.js';

/**
 * 
 * @param {AntetypeWeb} antetype oder so
 */
export function addCommands(antetype) {
    ...
    antetype.registerCommand("rebuildRenderObjectsForFigures", function (json, at) {
        ...
        at.rebuildRenderObjectsForFigures(newFigures);
    });
    ...
    antetype.registerCommand("changeProperty", function (json, at) {
        ...
        at.selectFigures(at.selectedObjects);
    ...
    let atShowsHandles;
    antetype.registerCommand("startPinch", function (json, at) {
        atShowsHandles = at._showHandles;
        if (atShowsHandles) {
            at.hideHandles();
            at.hideHandlesTemporarily();
        }
    });

    antetype.registerCommand("stopPinch", function (json, at) {
        if (atShowsHandles) {
            at.showHandles();
            at.showHandlesTemporarily();
        }
    });

    antetype.registerCommand("setCurrentZoom", function (zoomValue, at) {
        at.currentZoom = zoomValue;
        at.selectFigures(at.selectedObjects);
    });
    ...
    antetype.registerCommand("finishLoadProject", function (json, at) {
        if (at.runsInAntetype) {
            at.sendCommand("finishLoadProject");
        }
    });
    ...
    antetype.registerCommand("updateSmartGuides", function (guideCreatorNames, at) {
        at.enabledSmartGuides = guideCreatorNames;
    });

    antetype.registerCommand("disableTool", function (json, at) {
        at.disableSelectionOnCanvas();
    });

    // popover is closed, restore the selection tool if necessary:
    antetype.registerCommand("enableTool", function (json, at) {
        at.enableSelectionOnCanvas();
    });
}
